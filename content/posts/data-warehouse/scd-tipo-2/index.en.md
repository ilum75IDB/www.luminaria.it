...
1. **A {{< glossary term="chiave-surrogata" >}}surrogate key{{< /glossary >}}** — an identifier generated by the DWH, distinct from the source system's natural key. This is needed because the same customer will have multiple records (one per version), so the natural key is no longer unique.
...
A point that's often underestimated: the {{< glossary term="fact-table" >}}fact table{{< /glossary >}} must use the **surrogate key**, not the natural key.
...
That's the point. You don't implement Type 2 because "it's best practice" or because "Kimball says so in chapter 5." You implement it because a data warehouse without history is an operational database with a {{< glossary term="star-schema" >}}star schema{{< /glossary >}} bolted on top. It works for current month reports, but it can't answer the question that sooner or later someone will ask: "What was it like before?"
...
**[Surrogate key](/en/glossary/chiave-surrogata/)** — A numeric identifier generated by the data warehouse, distinct from the source system's natural key. In SCD Type 2 it's essential because the same record can have multiple versions, making the natural key no longer unique.

**[Fact table](/en/glossary/fact-table/)** — The central table in a star schema containing numeric measures (amounts, quantities, counts) and foreign keys to dimension tables. Each row represents a business event or transaction.

**[Kimball](/en/glossary/kimball/)** — Ralph Kimball, author of the data warehouse design methodology based on dimensional modeling, star schemas and bottom-up ETL processes. His framework classifies Slowly Changing Dimensions into types 0 through 7.

**[MERGE](/en/glossary/merge-sql/)** — A SQL statement that combines INSERT and UPDATE in a single operation: if the record exists it updates it, if it doesn't it inserts it. In Oracle it's also known as "upsert" and is the core ETL mechanism for SCD dimensions.

**[Star schema](/en/glossary/star-schema/)** — A data model typical of data warehouses: a central fact table connected to multiple dimension tables via foreign keys. It simplifies analytical queries and optimizes aggregation performance.
